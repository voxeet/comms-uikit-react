...
const Tooltip = ({ text, position, children, backgroundColor, textColor, testID, ...props }: TooltipProps) => {
  ...
  const tooltipPosition = useMemo(
    ...
    [position, wrapperRect, tooltipRect],
  );
    ...
    >
      {text && (
        <div
          ...
          ref={tooltipRef}
        >
          <div className={styles.content} style={{ backgroundColor: getColor(backgroundColor, colors.background) }}>
            <Text color={getColor(textColor, colors.white)} type="captionSemiBold">
              {text}
            </Text>
          </div>
        </div>
